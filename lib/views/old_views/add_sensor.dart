...
class AddSensorState extends State<AddSensor> {
  ...
  void initState() {
    ...
    final channel = WebSocketChannel.connect(
      Uri.parse(
          '$remoteWsDomain/api/v1/get-add-sensor-results?accessToken=${widget.accessToken}'),
    );
        ...
        if (data != null) {
          ...
          setState(() {
            data.forEach((key, value) {
              curState = SensorState.selectSensors;
              if (!sensors.map((k) => k.name).contains(key)) {
                found = true;
                              ...
                              onAdd: (SensorMaker maker, String fridgeID,
                                ...
                                () async {
                                  http.Response r = await http.post(
                                      Uri.parse(
                                          '$remoteHttpDomain/api/v1/finalize-add-sensor'),
